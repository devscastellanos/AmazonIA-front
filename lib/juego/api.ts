...
export function jugarCarta(accessToken: string, gameId: string, cardId: string, expectedVersion: number, signal?: AbortSignal): Promise<GameResponse> {
  return enviarComando(accessToken, gameId, { type: "play_card", cardId, expectedVersion }, signal);
}

export function descartarCarta(accessToken: string, gameId: string, cardId: string, expectedVersion: number, signal?: AbortSignal): Promise<GameResponse> {
  return enviarComando(accessToken, gameId, { type: "discard_card", cardId, expectedVersion }, signal);
}

export function finalizarTurno(accessToken: string, gameId: string, expectedVersion: number, signal?: AbortSignal): Promise<GameResponse> {
  return enviarComando(accessToken, gameId, { type: "end_turn", expectedVersion }, signal);
}

export async function crearPartida(accessToken: string, signal?: AbortSignal): Promise<GameResponse> {
